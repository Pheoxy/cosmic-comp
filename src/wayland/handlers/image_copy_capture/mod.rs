...
pub use self::user_data::{
    FrameHolder, ImageCopySessions, SessionData, SessionHolder, SessionOffscreen,
};
...
fn constraints_for_output(output: &Output, backend: &mut BackendData) -> Option<BufferConstraints> {
    ...
    let mut renderer = backend
        ...
        .unwrap();
    Some(match renderer.glow_renderer() {
        Some(glow) => constraints_for_renderer(mode, glow),
        None => constraints_shm_only(mode),
    })
}
...
fn constraints_for_toplevel(
    ...
    Some(match renderer.glow_renderer() {
        Some(glow) => constraints_for_renderer(size, glow),
        None => constraints_shm_only(size),
    })
}
